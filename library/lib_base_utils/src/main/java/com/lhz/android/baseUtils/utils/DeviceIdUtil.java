...
public class DeviceIdUtil {
    ...
    //保存文件的路径
    private static final String CACHE_UUID_DIR = "uuid/devices";
    //保存的文件 采用隐藏文件的形式进行保存
    private static final String DEVICES_FILE_NAME = "/device.uuid";
...
//        }
        ...
        //获取设备的MACAddress地址 去掉中间相隔的冒号 必须联网
        try {
...
//        return info.getMacAddress();
    ...
    private static File getDevicesDir(Context context) {
        ...
        if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
            File cropdir = new File(Environment.getExternalStorageDirectory(), CACHE_UUID_DIR);
            if (!cropdir.exists()) {
                cropdir.mkdirs();
            }
            mCropFile = new File(cropdir, DEVICES_FILE_NAME);
        } else {
            File cropdir = new File(context.getFilesDir(), CACHE_UUID_DIR);
            if (!cropdir.exists()) {
                cropdir.mkdirs();
            }
            mCropFile = new File(cropdir + DEVICES_FILE_NAME);
