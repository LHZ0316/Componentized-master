 ...
 */
public class LtoPermission {
    private final String TAG = "LtoPermission";
    private Context mContext;
    //自己的项目包名
    private static LtoPermission linToPermission;

    private LtoPermission(Context context) {
        this.mContext = context;
    }

    public static LtoPermission init(Context context) {
        if (linToPermission == null) {
            synchronized (LtoPermission.class) {
                if (linToPermission == null) {
                    linToPermission = new LtoPermission(context);
                }
            }
        }
    ...
    public void jumpPermissionPage() {
        ...
        Logger.getLogger("LtoPermission").warning("jumpPermissionPage --- name : " + name);
        switch (name) {
            case "HUAWEI":
                goHuaWeiMainager();
                break;
    ...
    private void goHuaWeiMainager() {
        try {
            ...
            Logger.getLogger("LtoPermission").warning(e.getMessage());
            e.printStackTrace();
            goIntentSetting();
        }
    }
    ...
    private void doStartApplicationWithPackageName(String packagename) {
        ...
        for (int i = 0; i < resolveinfoList.size(); i++) {
            Logger.getLogger("LtoPermission").warning(resolveinfoList.get(i).
                    activityInfo.packageName + resolveinfoList.get(i).activityInfo.name);
        }
